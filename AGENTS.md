...
- 不做 seed/expectsDatabaseQueryCount（依赖进程内容器）；数据库迁移与数据隔离由 `InteractsWithDatabase::setUpDatabase()` 提供（对应 laravel RefreshDatabase：$expect 安全校验 + $isolation 三隔离——truncate 跨进程 / transaction 单进程回滚 / memory 单进程内存库，细节见「目录结构」InteractsWithDatabase 条目）；InteractsWithDatabase 的定位是「测试进程直接连库断言」——sqlite 需用文件库（:memory: 仅存在于 server 进程内，测试进程连不上），DSN 用 `webmanRuntimePath()` 定位同源文件
...
- `src/`（按功能模块划分子目录，根目录仅保留核心入口；命名空间与目录一一对应，如 `Http/TestResponse.php` → `WebmanTech\Testing\Http\TestResponse`）：
  ...
  - `Concerns/`：能力按 laravel 同名 Concern 拆分组合（MakesHttpRequests/InteractsWithAuthentication/InteractsWithConsole/InteractsWithDatabase/InteractsWithServer）
    ...
    - `InteractsWithDatabase.php`：`setUpDatabase()` 数据库就绪（对应 laravel RefreshDatabase：$expect 安全校验 + $isolation 三隔离——truncate 跨进程 / transaction 单进程事务回滚 / memory 单进程 sqlite 内存库，transaction/memory 副作用由 TestCase::tearDown 自动清理）；自动注入连接：取被测应用 support\Db（webman/database，组件默认下游已安装）同源 PDO；PDO 实现 assertDatabaseHas/Missing/Count/Empty/SoftDeleted/NotSoftDeleted（表名/列名白名单校验 + bindValue 参数绑定防注入）；`setDatabaseConnection(new PDO(...))` 注入连接（对应 laravel 同名 Concern）
  - `Database/`：数据库就绪域（InteractsWithDatabase 依赖）
    - `MigratorInterface.php`：迁移器接口（migrate(): void，实现方保证幂等）
    - `PhinxMigrator.php`：phinx 实现（复用应用 phinx.php，class_exists 检测；可选 $connection 覆盖迁移目标连接（memory 模式迁到 :memory:，ArrayAccess 嵌套写无效需整体取出改回）；phinx 可选依赖，见 suggest）
    - `CallableMigrator.php`：callable 迁移器适配（testing 配置 database.migrator 传闭包时）
    - `TableCleaner.php`：清空业务表（sqlite 顺带重置自增 sqlite_sequence）
