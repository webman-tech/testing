...
/**
 * e2e install：完整安装应用（create-project → patch composer.json → update → reinstall → sync）
 */
final class InstallCommand extends Command
{
    ...
    {
        $this->setDescription('完整安装 e2e 应用（create-project → patch composer.json → update → reinstall → sync）')
            ->addArgument('app', InputArgument::OPTIONAL, '应用名（省略时安装定义中全部应用）')
            ->addOption('vcs', null, InputOption::VALUE_NONE, '被测包经 GitHub VCS 安装（而非本地 path repository）')
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        foreach ($apps as $app) {
            $installer->install($app, ['vcs' => (bool)$input->getOption('vcs')]);
        }
